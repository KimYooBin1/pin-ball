...
export class RankRenderer implements UIObject {
  ...
  onDblClick(e?: MouseEventArgs) {
    if (e && navigator.clipboard) {
      ...
      tsv.push(
        ...this.winners.map((m) => {
          rank++;
          return [rank.toString(), m.name, rank - 1 === this.winnerRank ? 'WINNER' : 'FINISHED'].join('\t');
        }),
        ...this.marbles.map((m) => {
          rank++;
          return [rank.toString(), m.name, 'RUNNING'].join('\t');
        }),
        ...this.eliminated.map((m) => ['-', m.name, 'OUT'].join('\t'))
      );

      tsv.unshift(['Rank', 'Name', 'Status'].join('\t'));
  ...
  render(
    ...
    { winners, marbles, eliminated, winnerRank, theme }: RenderParameters,
    width: number,
    height: number
  ) {
    const startX = width - 5;
    const startY = Math.max(-this.fontHeight, this._currentY - height / 2);
    const totalEntries = marbles.length + winners.length + eliminated.length;
    this.maxY = Math.max(0, totalEntries * this.fontHeight + this.fontHeight);
    ...
    ctx.fillStyle = '#666';
    ctx.fillText(`${winners.length} / ${totalEntries}`, width - 5, this.fontHeight);
    ...
    eliminated.forEach((marble, index) => {
      const y = (index + winners.length + marbles.length) * this.fontHeight;
      if (y >= startY && y <= startY + ctx.canvas.height) {
        ctx.fillStyle = '#ff7f7f';
        ctx.strokeText(`OUT ${marble.name}`, startX, 20 + y);
        ctx.fillText(`OUT ${marble.name}`, startX, 20 + y);
      }
    });
    ctx.restore();
  }
